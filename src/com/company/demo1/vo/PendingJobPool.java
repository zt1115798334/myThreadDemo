...
public class PendingJobPool {
    ...
    private static class PendingTask<T, R> implements Runnable {
        ...
        public void run() {
            R r = null;
            ITaskProcesses<T, R> taskProcesses = (ITaskProcesses<T, R>) jobInfo.getTaskProcesses();
            TaskResult<R> result = null;
            try {
                //调用业务人员实现的具体方法
                result = taskProcesses.taskExecute(processData);
                //要做检查，防止开发人员处理不当
                if (result == null) {
    ...
    public <R> void registerJob(String jobName, int jobLength,
                                ITaskProcesses<?, ?> taskProcesses, long expireTime) {
        JobInfo<R> jobInfo = new JobInfo<>(jobName, jobLength,
                taskProcesses, expireTime);
